...
```json
{
  "id": "角色ID（由调用方指定，原样填入，不可修改）",
  "name": "中文名",
  "gender": "male/female",
  "appearance": "详细外貌描述（50-100字），包含年龄、发型、五官、体型、身高等",
  "personality": "性格特征简述（20-40字）",
  "outfits": {
    "default": "默认服装的详细描述（30-50字）",
    "casual": "休闲装描述（30-50字）",
    "formal": "正装描述（30-50字）"
  },
  "voice": {
    "key": "声音标识（如 male-1, female-warm-2 等简短代号）",
...
- voice.key 为简短声音标识代号，voice.voice_description 为详细描述
- 只输出 JSON，不要额外文字"""


def _normalize_character(char: dict) -> dict:
    """规范化角色数据，确保 voice / outfits 格式符合前端预期"""
    # voice: 确保有 key 字段
    ...
    if isinstance(voice, dict):
        ...
        gender = char.get("gender", "")
        prefix = "male" if gender == "male" else "female" if gender == "female" else "voice"
        char["voice"] = {"key": f"{prefix}-1", "voice_description": ""}

    # outfits: 确保有 default 键
